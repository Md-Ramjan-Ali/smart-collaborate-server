...
const createProject = async (
  ...
  if (existingProject) {
    throw new AppError(400, 'Project title must be unique!');
  }
 ...
 */
const getAllProjects = async (userId: string, role: Role) => {
  if (role === Role.ADMIN) {
 ...
 */
const getProjectById = async (projectId: string, userId: string, role: Role) => {
  ...
  if (role !== Role.ADMIN && project.ownerId !== userId) {
    ...
    if (!isMember) {
      throw new AppError(403, 'You do not have permission to view this project!');
    }
  }
...
const updateProject = async (
  projectId: string,
  payload: Partial<Project> & { teamMembers?: string[] },
  userId: string,
  role: Role
) => {
  const project = await prisma.project.findUnique({
 ...
 */
const addTeamMember = async (projectId: string, memberId: string, userId: string, role: Role) => {
  const project = await prisma.project.findUnique({
    where: { id: projectId },
  ...
  if (role !== Role.ADMIN && project.ownerId !== userId) {
    throw new AppError(403, 'You do not have permission to invite members to this project!');
  }
 ...
 */
const deleteProject = async (projectId: string, userId: string, role: Role) => {
  const project = await prisma.project.findUnique({
    where: { id: projectId },
  });
